function [] = run_single_decoding_generalization_reshuffles(monkey, area, arbitrary_cell_no, ref, align, time_start, time_window, time_type, ...
...
num_reshuffles= params.num_reshuffles;
delete_old_reshuffles=params.delete_old_reshuffles;

if ~reshuffle; error('This function should only be called to run reshuffles'); end
...
file_prefix=['CVs_' num2str(num_cv_splits) '_resamples_' num2str(num_resample_runs) '_'];    
...
dir_list=dir(results_dir);
TF = strncmp(file_prefix,{dir_list.name},length(file_prefix));

if delete_old_reshuffles
    TF2 = strncmp(['params_' file_prefix],{dir_list.name},length(['params_' file_prefix]));
    temp_shit={dir_list.name};
    temp_shit=temp_shit(TF | TF2);
    for i=1:length(temp_shit)
        delete(fullfile(results_dir,temp_shit{i}));
    end
    index_offset=0;
else
    index_offset=sum(TF);
...
for reshuffle_iteration=index_offset+(1:num_reshuffles)
    clear ds the_cross_validator %Just in case, to make sure a different reshuffle is used on each iteration
    ...
    results.sample_sites_with_replacement = sample_sites_with_replacement;
        ...
        %fullfile(results_dir, [file_prefix save_file_name])
        save(fullfile(results_dir, [file_prefix save_file_name sprintf('%06.0f',reshuffle_iteration)]), '-v7.3', 'DECODING_RESULTS');
        
        save(fullfile(results_dir,['params_' file_prefix save_file_name sprintf('%06.0f',reshuffle_iteration) '.mat']), 'results');
        
        cd(results_dir);
        
        fileID = fopen(fullfile(results_dir,['params_' file_prefix save_file_name sprintf('%06.0f',reshuffle_iteration) '.txt']), 'w');
