function [] = create_population_folder_func(monkey)
...
for area_index = 1 : 2
    ...
    for ref_index = 1 : length(ref)
        ...
        curr_ref = fix_ref_case(ref{ref_index});
        
        if strcmpi(curr_ref, 'saccade')
            time_start = -800;%-1500;
        ...
        if strcmpi(time_type, 'window')
            align_str = [curr_ref '_' align '_' num2str(time_start) '_' num2str(time_window) '_' lower(time_type) '_clean'];
        else
            align_str = [curr_ref '_' align '_' num2str(time_start) '_' lower(time_type) '_clean'];
        end

        pop_dir = fullfile('/Freiwald/ppolosecki', 'lspace', 'plevy', 'data', monkey, 'attn', ['Population_' area{area_index} '_' curr_ref], align_str);
        ...
        for cell_no = 1 : 55
            ...
            if ~exist(full_dir, 'dir')
                warning('Directory containing results not found. Cleaning of neural data for %s %s %s %s-aligned cell %d likely not completed.', ...
                                                            monkey, area{area_index}, curr_ref, align, cell_no);
                continue;
            end;
