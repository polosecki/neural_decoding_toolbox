function [ts,te,inclidx] = find_ref_points_test(fid,tmap,start_trig,end_trig,surf_str)
% Find the intervals of interest based on trigger points and offsets

global DH 

excl = zeros(length(tmap.ts),1);
ideal_trials=strcmp({surf_str.out},'Success')' & strcmp({surf_str.type},'Normal')';
...
elseif strcmp(start_trig.type,'Marker')
    ...
    [ts,excl] = proc_marker(tmap,mt,excl,ideal_trials);%,surf_str);
else
...
elseif strcmp(end_trig.type,'Marker')
    ...
    %keyboard
    [te,excl] = proc_marker(tmap,mt,excl,ideal_trials);%,surf_str);
    sum(ideal_trials(logical(excl)))%keyboard
    %keyboard
...
function [tp,excl] = proc_marker(tmap,mt,excl,ideal_trials)%,surf_str)
tp = zeros(length(tmap.ts),1);
%ttt=[];
for i=1:length(tmap.ts)
    ...
    if length(idx) ~= 1
        excl(i) = 1;
        if ideal_trials(i) & length(idx)>1
            excl(i) = 0;
            idx=idx(end);
            tp(i) = mt(idx);
            disp('fixed')
            %if length(idx)>2; disp('more than 2');end
%            ttt=[ttt;diff(mt(idx)/1e9)];
        end
    else
        tp(i) = mt(idx);
    end
