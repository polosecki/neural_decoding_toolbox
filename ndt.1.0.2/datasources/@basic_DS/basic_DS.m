...
%                error('sites_to_use can not be empty')
                warning('No valid sites (perhaps not enough data! Analysis not conducted.');
                [XTr_all_time_cv, YTr_all, XTe_all_time_cv, YTe_all] = deal([]);
                return;
            end
...
%                                 '(this can be done by setting ds.site_to_use = find_sites_with_at_least_k_repeats_of_each_label(the_labels_to_use, num_cv_splits) )']);  
                            warning('Not enough data! Certain conditions not repeated enough.');
                            [XTr_all_time_cv YTr_all XTe_all_time_cv YTe_all] = deal([], [], [], []); %Paul error code
                            return;
                        else
