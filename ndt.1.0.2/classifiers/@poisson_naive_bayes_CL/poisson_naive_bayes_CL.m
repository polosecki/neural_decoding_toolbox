  ...
  properties 
    ...
    methods 
        ...
        function cl = train(cl, XTr, YTr)  
           ...
           if sum(sum(abs(round(XTr) - XTr))) > 10.^-8  % (not having it be exactly zero to account for round off error)
        ...
        function [predicted_labels decision_values] = test(cl, XTe)
           ...
           if sum(sum(abs(round(XTe) - XTe))) > 10.^-8   % (not having it be exactly zero to account for round off error)
