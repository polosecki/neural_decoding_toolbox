...
%==========================================================================    
  ...
  properties 
      ...
      use_fewer_NULL = 0; %Ethan's fix to the NDT that requires fewer null distributions (~5 rather than 200) to accurately compute significance
    ...
    methods 
        ...
        function the_figure = plot_results(plot_obj)
            ...
            for iResult = 1:number_of_results_to_plot
               ...
               if isstruct(curr_time_interval) || ~isempty(curr_default_binning_parameters) && ~(ismatrix(curr_time_interval)) && ~isempty(curr_time_interval)
                ...
                the_figure = plot(curr_time_interval(curr_plot_inds), curr_result_to_plot(curr_plot_inds), plot_obj.line_style, 'LineWidth', plot_obj.line_width, 'Color', plot_obj.the_colors{iResult}); 
                hold on    
            ...
            if ~isempty(plot_obj.errorbar_file_names)
                ...
                % more sanity checks
                if size(plot_obj.errorbar_file_names, 1) ~= number_of_results_to_plot     %length(plot_obj.errorbar_file_names) ~= number_of_results_to_plot
                ...
                for iResult = 1:number_of_results_to_plot
                    ...
                    if isstr(plot_obj.p_values{iResult})
                      ...
                      %   (and also trusting that the user knows what he/she is doing) but not going to do this right now).
                       if ismatrix(plot_obj.result_file_names) && ~ismatrix(plot_obj.p_values{iResult})
                          error(['If plot_obj.result_file_names is a [num_results x num_time_bins] matrix with precomputed decoding accuracies ' ...
                              'then one can not computer vaules by having plot_obj.p_values{iResult} contain the name of a directory']); 
                       end
                                                
%                        fprintf('Creating p value object from %s\n', plot_obj.p_values{iResult});
                       ...
                       pval_obj.collapse_all_times_when_estimating_pvals = use_fewer_NULL;
                       plot_obj.p_values{iResult} = pval_obj.create_pvalues_from_nulldist_files();
                       
                       if isempty(plot_obj.p_values{iResult})
%                            error('P value analysis failed!');
                           return;
                       end
