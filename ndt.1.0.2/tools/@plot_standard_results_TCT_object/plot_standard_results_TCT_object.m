...
%==========================================================================    
  ...
  properties 
    ...
    methods 
        ...
        function [min_and_max_data_range, the_figure] = plot_results(plot_obj)
            ...
            if isstr(plot_obj.result_file_name)
                ...
                if isempty(all_results_data)
%                     warning('Invalid decoding');
                    [min_and_max_data_range, the_figure] = deal([]);
                    return;
                end
...
%                 if ~isstruct(the_time_interval)
%                     error('Mistake in NDT!');
%                 end
                ...
                if isempty(time_interval_obj.end_time)
                    the_time_interval = the_time_interval(1:length(result_to_plot)); 
                end
            ...
            addpath('/Freiwald/ppolosecki/lspace/plevy/helper_code/bipolar_colormap.zip_FILES/bipolar_colormap');

            % display the results
            the_figure = figure(plot_obj.TCT_figure_number);
            hold off

%             colormap(bipolar());
            load('/Freiwald/ppolosecki/lspace/plevy/helper_code/bipolar_colormap.zip_FILES/bipolar_colormap/CoolWarmMap.mat');
            colormap(cmap);
            ...
            for iEvent = 1:length(plot_obj.significant_event_times)
               ...
               line(get(gca, 'XLim'), [significant_event_inds(iEvent), significant_event_inds(iEvent)], 'color', [0 0 0])
            end
