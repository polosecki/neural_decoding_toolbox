function saved_binned_data_file_name = create_binned_data_from_raster_data(raster_file_directory_name, save_prefix_name, bin_width, sampling_interval, is_pop, is_rel, start_time, end_time)
...
if isempty(raster_file_dir)
    if isempty(raster_file_dir)
        error('The directory name (%s) given does not exist', raster_file_directory_name);
    else
        error('The directory name (%s) given does not contain any .mat files', raster_file_directory_name);
    end
end
...
if (length(sampling_interval) > 1) && (nargin > 6)
...
%     fprintf('Looking at file %s\n', raster_file_dir(i).name);
    
    if is_pop == 1 && is_rel == 1
        if ~isempty(strfind(raster_file_dir(i).name, 'shift')) %Paul: If relative analysis, skip psuedo-population cells
            continue;
        end
    end
    
    if is_pop == 0
        if ~isempty(strfind(raster_file_dir(i).name, 'shift')) %Paul: If not population analysis, skip psuedo-population cells
            continue;
        end
    end
   ...
   binned_data{cell_index} = curr_binned_data;
     
   % save all the labels
   the_label_names = fieldnames(raster_labels);
   for iLabel = 1:length(the_label_names)
      eval(['binned_labels.' the_label_names{iLabel} '{cell_index} = raster_labels.' the_label_names{iLabel} ';']); 
   end
   ...
   if ~isempty(raster_site_info)
       ...
       the_info_field_names = fieldnames(raster_site_info);  
        ...
        for iInfo = 1 : length(the_info_field_names)
          ...
          %Update time-vector to account for sampling interval before saving
          if strcmpi(the_info_field_names{iInfo}, 'time_vector');
              time_vector = eval(['raster_site_info.' the_info_field_names{iInfo} ';']);
              updated_time_vector = zeros(1, floor(size(time_vector, 2)/sampling_interval));
              for count = 1 : size(updated_time_vector, 2)
                  updated_time_vector(1, count) = time_vector(1, (count-1)*sampling_interval + 1);
              end
              eval(['raster_site_info.' the_info_field_names{iInfo} '= updated_time_vector;']);
          end
                     
          if isstr( eval(['raster_site_info.' the_info_field_names{iInfo}]))
              eval(['binned_site_info.' the_info_field_names{iInfo} '{cell_index} = raster_site_info.' the_info_field_names{iInfo} ';']); 
          elseif isempty( eval(['raster_site_info.' the_info_field_names{iInfo}]))
             % just ignore this field if it is empty...
          else
              eval(['binned_site_info.' the_info_field_names{iInfo} '{cell_index, :} = raster_site_info.' the_info_field_names{iInfo} ';']);   
              % might run into problems with this so above line could be
              % more useful (author's comment)
              %IMPORTANT: Paul has changed the above from matrix to cell in
              %order to fix problem of differently sized elements
              %(the problem was anticipated by author of NDT!!!)
              %This may cause problems, but likely will not.
          end
  ...
  cell_index = cell_index + 1; %If we've made it here, we've added one more cell
...
if (nargin < 7) && (length(bin_width) == 1) && (length(sampling_interval) == 1) 
    saved_binned_data_file_name = [save_prefix_name '_' num2str(bin_width) 'ms_bins_' num2str(sampling_interval) 'ms_sampled'];     
elseif (nargin > 6) && (length(bin_width) == 1) && (length(sampling_interval) == 1)  
...
%NOTE: Paul has added 'binned' directory to save path
if ~exist(fullfile(raster_file_directory_name, 'binned'), 'dir')
    mkdir(fullfile(raster_file_directory_name, 'binned'))
end

% fprintf('Saving the binned data to file %s\n', fullfile(raster_file_directory_name, 'binned', saved_binned_data_file_name));

save(fullfile(raster_file_directory_name, 'binned', saved_binned_data_file_name), '-v7.3', 'binned_data', 'binned_labels', 'binned_site_info');
