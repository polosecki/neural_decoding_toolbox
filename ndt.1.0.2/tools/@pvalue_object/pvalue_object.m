...
%            result would have been that low by chance). If this is set to any other value, a two-tailed test is performed relative to mean chance decoding. (P.P. 1/10/2015)       
%         
%
        ...
        function [p_values null_distributions PVALUE_PARAMS] = create_pvalues_from_nulldist_files(pval_obj)
            ...
            if pval_obj.collapse_all_times_when_estimating_pvals == 1       % added with NDT version 1.4 - creates null distribution using data points from all points in time
                ...
                for iTime = 1:size(pval_obj.null_distributions, 2)
                    ...
                    if pval_obj.real_decoding_results_lower_than_null_distribution==1   % if real_decoding_results_lower_than_null_distribution == 1, see if real results are less than null distirbution values
                    ...
                    elseif pval_obj.real_decoding_results_lower_than_null_distribution==0
                        pval_obj.p_values(iTime) = length(find(total_null_distribution_collapsed_across_time >= pval_obj.real_results(iTime)))./length(total_null_distribution_collapsed_across_time);
                    else %2-Tailed test, added by P.P. 1/10/2015
                        mean_null_decoding=mean(total_null_distribution_collapsed_across_time(:));
                        pval_obj.p_values(iTime) = length(find(abs(total_null_distribution_collapsed_across_time-mean_null_decoding) >= abs(pval_obj.real_results(iTime)-mean_null_decoding)))./length(total_null_distribution_collapsed_across_time);
                    end
                end
                ...
                for iTime = 1:size(pval_obj.null_distributions, 2)
                    ...
                    elseif pval_obj.real_decoding_results_lower_than_null_distribution==0
                        ...
                        mean_null_decoding(iTime)=mean(pval_obj.null_distributions(:, iTime));
                        pval_obj.p_values(iTime) = length(find(abs(pval_obj.null_distributions(:, iTime)-mean_null_decoding(iTime)) >= abs(pval_obj.real_results(iTime)-mean_null_decoding(iTime))))./size(pval_obj.null_distributions, 1);
