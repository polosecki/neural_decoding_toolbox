...
%==========================================================================   
    ...
    properties 
        ...
        confusion_matrix_params = struct('create_confusion_matrix', 1, 'create_all_test_points_separate_confusion_matrix', 0, ...
                                    'save_confusion_matrix_only_train_and_test_at_same_time', 1, 'save_all_resamples', 1);
                            %Paul has added the 'save_all_resamples' parameter
        
        % flags for displaying the progress of the decoding procedure    
        % EDIT: Paul has changed value of resample_run_time and
        %       zero_one_loss to 0 to suppress output!
        display_progress = struct('resample_run_time', 0, 'zero_one_loss', 0, 'normalized_rank', 0, 'decision_values', 0, 'separate_CV_ROC_results', 0, ...
    ...
    methods
        ...
        function DECODING_RESULTS = run_cv_decoding(cv) 
            ...
            for iResample = 1:cv.num_resample_runs   
                ...
                if isempty(all_XTr) && isempty(all_YTr) && isempty(all_XTe) && isempty(all_YTe) %Paul error check
                    warning('Quititng without analysis.');
                    DECODING_RESULTS = []; %Paul error code
                    return;
                end
