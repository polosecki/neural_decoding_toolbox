...
%==========================================================================   
    ...
    if cv.confusion_matrix_params.save_all_resamples == 1
        
        if cv.confusion_matrix_params.save_confusion_matrix_only_train_and_test_at_same_time == 1
            all_resampled_CM = zeros(num_labels, num_labels, num_train_time_intervals, num_resamples);
        else
            all_resampled_CM = zeros(num_labels, num_labels, num_train_time_intervals, num_test_time_intervals, num_resamples);
        end
    end
    
%     dims = size(confusion_matrix);
%     fprintf('Start: Confusion matrix has %d dims: [', length(dims));
...
%     fprintf('%d train, %d test intervals\n', num_train_time_intervals, num_test_time_intervals);
   ...
   for iTrain = 1:num_train_time_intervals
        for iTest = 1:num_test_time_intervals
            ...
            for iResample = 1:num_resamples
                ...
                % if actually saving the resamples
                if cv.confusion_matrix_params.save_all_resamples == 1
                        ...
                        if cv.confusion_matrix_params.save_confusion_matrix_only_train_and_test_at_same_time == 1 && (num_train_time_intervals == num_test_time_intervals) 
                            if iTrain == iTest   
                                all_resampled_CM(:, :, iTrain, iResample) = curr_resample_CM;
                            end
                        ...
                        elseif cv.confusion_matrix_params.save_confusion_matrix_only_train_and_test_at_same_time == 1  && (num_train_time_intervals ~= num_test_time_intervals) 
                            all_resampled_CM(:, :, iTrain, iResample) = all_resampled_CM(:, :, iTrain, iResample) + curr_resample_CM;
                            
                        else
                            all_resampled_CM(:, :, iTrain, iTest, iResample) = curr_resample_CM;
                        end
                end
...
%         dims = size(confusion_matrix);
%         fprintf('End: Confusion matrix has %d dims: [', length(dims));
%         for i = 1 : length(dims) - 1
%             fprintf('%d X ', dims(i));
%         end
%         fprintf('%d]\n', dims(length(dims)));
    ...
    if cv.confusion_matrix_params.save_all_resamples == 1 %PAUL
        DECODING_RESULTS.ZERO_ONE_LOSS_RESULTS.confusion_matrix_results.all_resamples = all_resampled_CM;
    end
